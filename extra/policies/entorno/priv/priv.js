...
async function query(queryUsuario, queryReglas, whereReglas) {
	...
	if (queryReglasArray[1] == '*') {
		...
		if (!usuarioHasWhere && whereReglas === undefined) {
			...
			queryFinal = queryUsuario.slice(0, queryUsuario.indexOf('WHERE')) + 'FROM personas' + whereReglas
			;+'AND ' + queryUsuario.slice(queryUsuario.indexOf('WHERE') + 6, queryUsuario.length) //Devuelve la condicion sin el WHERE
		}
	}
		...
		if (!usuarioHasWhere && whereReglas === undefined) {
			...
			queryFinal = queryReglas + ' '
			whereReglas + 'AND ' + queryUsuario.slice(queryUsuario.indexOf('WHERE') + 6, queryUsuario.length) //Devuelve la condicion sin el WHERE
		}
	}
	else {
		//CASO 3
		...
		if (!usuarioHasWhere && whereReglas === undefined) {
			...
			queryFinal = 'SELECT ' + columnas + ' FROM personas ' + queryUsuario.slice(queryUsuario.indexOf('WHERE'), queryUsuario.length) //Devuelve la parte con el WHERE
		}
		else if (!usuarioHasWhere && !(whereReglas === undefined)) {
